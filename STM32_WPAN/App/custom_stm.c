...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomLmserviceHdle;                    /**< LiquidMonitoringService handle */
  uint16_t  CustomHeightHdle;                  /**< height handle */
  uint16_t  CustomColorHdle;                  /**< color handle */
  uint16_t  CustomHeightcalHdle;                  /**< heightCalibration handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeHeight = 8;
uint16_t SizeColor = 6;
uint16_t SizeHeightcal = 16;
...
#define COPY_LIQUIDMONITORINGSERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_HEIGHT_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_COLOR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x02,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_HEIGHTCALIBRATION_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x03,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomHeightHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_HEIGHT_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_HEIGHT_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomColorHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_COLOR_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_COLOR_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomColorHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/

          else if (attribute_modified->Attr_Handle == (CustomContext.CustomHeightcalHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            /* USER CODE BEGIN CUSTOM_STM_Service_1_Char_3_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
            Custom_App_StartCalibration(attribute_modified->Attr_Data, attribute_modified->Attr_Data_Length);
            /* USER CODE END CUSTOM_STM_Service_1_Char_3_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
          } /* if (attribute_modified->Attr_Handle == (CustomContext.CustomHeightcalHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for LiquidMonitoringService +
   *                                2 for height +
   *                                2 for color +
   *                                2 for heightCalibration +
   *                                1 for height configuration descriptor +
   *                                1 for color configuration descriptor +
   *                              = 9
   *
  ...
  COPY_LIQUIDMONITORINGSERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomLmserviceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: lmService, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: lmService \n\r");
  }

  /**
   *  height
   */
  COPY_HEIGHT_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomLmserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeHeight,
                          ...
                          &(CustomContext.CustomHeightHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : HEIGHT, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : HEIGHT \n\r");
  }
   ...
   *  color
   */
  COPY_COLOR_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomLmserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeColor,
                          CHAR_PROP_READ | CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomColorHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : COLOR, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : COLOR \n\r");
  }
   ...
   *  heightCalibration
   */
  COPY_HEIGHTCALIBRATION_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomLmserviceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeHeightcal,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomHeightcalHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : HEIGHTCAL, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : HEIGHTCAL \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_HEIGHT:
      ret = aci_gatt_update_char_value(CustomContext.CustomLmserviceHdle,
                                       CustomContext.CustomHeightHdle,
                                       0, /* charValOffset */
                                       SizeHeight, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value HEIGHT command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value HEIGHT command\n\r");
      }
    ...
    case CUSTOM_STM_COLOR:
      ret = aci_gatt_update_char_value(CustomContext.CustomLmserviceHdle,
                                       CustomContext.CustomColorHdle,
                                       0, /* charValOffset */
                                       SizeColor, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value COLOR command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value COLOR command\n\r");
      }
    ...
    case CUSTOM_STM_HEIGHTCAL:
      ret = aci_gatt_update_char_value(CustomContext.CustomLmserviceHdle,
                                       CustomContext.CustomHeightcalHdle,
                                       0, /* charValOffset */
                                       SizeHeightcal, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value HEIGHTCAL command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value HEIGHTCAL command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_HEIGHT:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomLmserviceHdle, CustomContext.CustomHeightHdle, SizeHeight, pPayload);
    ...
    case CUSTOM_STM_COLOR:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomLmserviceHdle, CustomContext.CustomColorHdle, SizeColor, pPayload);
    ...
    case CUSTOM_STM_HEIGHTCAL:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomLmserviceHdle, CustomContext.CustomHeightcalHdle, SizeHeightcal, pPayload);
